...
//Thirst
	...
	if (_messTimer > 15) then {
        _messTimer = 0;
        if (ori_morpht > 0 and r_fracture_legs) then {
            player setVariable["morph_t", [ori_morpht, ori_morpht_max], (1 > 0)];
        };
    };
    if (ori_morpht > 0) then {
        ori_morpht = (ori_morpht - 2) max 0;
		hintsilent format ["ori_morpht: %1 r_fracture: %2",ori_morpht,r_fracture_legs];
        if (ori_morpht == 0 and r_fracture_legs) then {
            player setvariable["morph_t", [], (1 > 0)];
            player setHit["legs", 1];
        };
    };
	//call fnc_calculation;
	//Save Checker
